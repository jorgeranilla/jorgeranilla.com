...
/* ===========================
   FEEDBACK WIDGET
=========================== */
// Load feedback widget HTML (embedded to avoid CORS issues with file:// protocol)
  ...
  <div id="feedbackModal" class="feedback-modal">
    <div class="feedback-modal-content">
      <div class="feedback-modal-header">
        ...
        <button id="closeFeedbackModal" class="feedback-close-btn" aria-label="Close">&times;</button>
      </div>
      <div class="feedback-modal-body">
        ...
        <form id="feedbackForm">
          <div class="feedback-form-group">
            <label for="issueType">Feedback Type</label>
            <select id="issueType" name="issueType" required>
              <option value="">Select a type...</option>
              <optgroup label="Issues">
                <option value="broken-link">Broken Link</option>
                <option value="missing-image">Missing/Broken Image</option>
                <option value="layout-issue">Layout Problem</option>
          ...
          <div class="feedback-form-group">
            <label for="issueDescription">Your Message</label>
            <textarea id="issueDescription" name="description" rows="4" placeholder="Share your thoughts..." required></textarea>
          </div>
          
          <div class="feedback-form-group">
            <label for="pageUrl">Page URL</label>
            <input type="text" id="pageUrl" name="pageUrl" readonly>
          </div>
          
          <div class="feedback-form-actions">
            <button type="button" id="cancelFeedback" class="feedback-btn feedback-btn-secondary">Cancel</button>
            <button type="submit" class="feedback-btn feedback-btn-primary">Send Feedback</button>
          </div>
        </form>
        ...
        <div id="feedbackSuccess" class="feedback-success" style="display: none;">
          ...
          <button id="closeSuccessMsg" class="feedback-btn feedback-btn-primary">Close</button>
        </div>
      </div>
    </div>
  </div>`;
...
function initializeFeedbackWidget() {
  ...
  const closeSuccessMsg = document.getElementById('closeSuccessMsg');
  ...
  feedbackForm.addEventListener('submit', async (e) => {
    ...
    submitData.append('Issue Type', formData.get('issueType'));
    submitData.append('Description', formData.get('description'));
    submitData.append('Page URL', formData.get('pageUrl'));
    submitData.append('Timestamp', new Date().toLocaleString());
    submitData.append('User Agent', navigator.userAgent);

    try {
      // Send to FormSubmit
      const response = await fetch('https://formsubmit.co/el/fupeko', {
        method: 'POST',
        body: submitData,
        headers: {
      ...
      if (response.ok) {
        ...
        // If FormSubmit fails, show success anyway (better UX)
  ...
  closeSuccessMsg.addEventListener('click', closeModal);
}
